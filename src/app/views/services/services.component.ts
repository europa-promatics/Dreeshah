...
export class ServicesComponent implements OnInit {
  ...
  options: Options = {
    ...
    translate: (value: number, label: LabelType): string => {
      switch (label) {
        case LabelType.Low:
          return "<b>Min price:</b> AED" + value;
        case LabelType.High:
          return "<b>Max price:</b> AED" + value;
        default:
          return "AED" + value;
      }
    }
  ...
  ngOnInit() {
    ...
    if (this.category_id && this.sub_category_id) {
      ...
      this.getAllServiceList(this.searchFilters)
    }
  ...
  getAllServiceList(obj) {
    ...
    if (obj.min_price == null) {
      delete obj.min_price
    }
    if (obj.max_price == null) {
      delete obj.max_price
    }

    if (obj.service_sub_category == null) {
      delete obj.service_sub_category
    }

    if (obj.service_category == null) {
      delete obj.service_category
    }
  ...
  getCatAndSubCats() {
    ...
    this.CustomerService.getCatAndSubCat().subscribe(data => {
      console.log("Response of all Category and sub-category listing is >>>>>>", data);
      if (data.code == 200) {
        ...
        let temp1 = []
        data.data[0].sub_categories.forEach((d) => {
          d.checked = false
          temp1.push(d)
        })
  ...
  getstyles() {
    this.CustomerService.getStylelisting().subscribe(data => {
      console.log("Response of All styles is =>>>>>", data);
      if (data.code == 200) {
  ...
  getDuration() {
    this.CustomerService.getDurationlisting().subscribe(data => {
      console.log("Response of All Duration =>>>>>", data);
      if (data.code == 200) {
  ...
  selectedServiceCategory(e: any, _id: string) {
    if (e.target.checked) {
      this.data.push(_id);
    ...
    this.services_list_all = this.data
    this.searchFilters.service_category = this.data
  ...
  selectedServiceSubCategory(e: any, _id: string) {
    ...
    if (e.target.checked) {
      ...
      var ind = this.sdata.indexOf(_id);
      if (ind > -1) {
        this.sdata1 = this.sdata.splice(ind, 1)
      }
    }
    this.services_list_all = this.sdata,
      this.searchFilters.service_sub_category = this.sdata
    this.getAllServiceList(this.searchFilters);
  }
  ...
  sliderEvent() {
    ...
    this.services_list_all = this.data
    this.searchFilters.min_price = price_together[0]
    this.searchFilters.max_price = price_together[1]
    console.log('Data is ===>', this.data);
    this.getAllServiceList(this.searchFilters);
  }
  sort(event: any) {
    switch (event.target.value) {
      case "Low":
        {
          this.services_list_all = this.services_list_all.sort((low, high) => low.price - high.price);
          break;
        }

      case "High":
        {
          this.services_list_all = this.services_list_all.sort((low, high) => high.price - low.price);
          break;
        }
  ...
  bookAppointment() {
    ...
    if (localStorage['userData']) {
      this.userData = JSON.parse(localStorage['userData']);
      console.log("userappoint======>", this.userData)
      if (this.userData.user_type != "customer") {
        this.toastr.error("Please login as customer to continue")
        return
      } else {
        ...
        var obj = {
          ...
          date: this.bookForm.value.appDate,
          time: this.bookForm.value.appTime,
